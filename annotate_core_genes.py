...
def load_csv_file(filename = "", acc_idx_array = [i for i in range(4,57)], gene_idx = 3):
  pav_dict = dict()
  with open(filename) as fh:
    next(fh)
    for line in fh:
      line_array = line.strip().split(",")
      #skip if gene is labeled "BnaPangenome" because we do not have annotations for these
      #wait.. then how did we calculate ka/ks?
      #ahh, I just put 99
      if re.match("^BnaPanG",line_array[gene_idx]):
        continue
      
      #select specific accessions from header
      #define this gene as core or dispenable      
      if set([line_array[i] for i in acc_idx_array]) == {"PRESENT"}:
        pav_dict[line_array[gene_idx].replace('.1','')] = "Core"
      else:
        pav_dict[line_array[gene_idx].replace('.1','')] = "Dispensable"
    ...
    for line in fh:
      ...
      if re.match("mRNA",line_array[2]):
        ...
        if len(cg_breaks) == 0:
          ...
          try:
            pav_dict[cg].extend((cg_aed, 
            				cg_length, cg_exon_count, cg_exon_length,
            				cg_intron_length))
          except KeyError:
      ...
      #make new 2d array
        try:
          cg = re.search("Name=([a-zA-Z0-9\.]*);",line_array[8]).group(1)
          cg = cg.replace('.1','')
        except:
          cg = re.search("Name=(.*)",line_array[8]).group(1)
          cg = cg.replace('.1','')
        try:
          #cg_aed = re.search("_eAED=(.*);|$",line_array[8]).group(1)
          cg_aed = re.search("_eAED=([a-zA-Z0-9\.]*);",line_array[8]).group(1)
        except:
          cg_aed = re.search("_eAED=([a-zA-Z0-9\.]*)",line_array[8]).group(1)
...
def add_aa_table(filename = "/mnt/research/edgerpat_lab/AlanY/00_collab/02_core_pred/01_reference/bna_aa_table.txt", pav_dict = dict()):
  failed = 0
  with open(filename) as fh:
    for line in fh:
      line_array = line.strip().split("\t")
...
if __name__ == "__main__":
  #load feature files
  pav_dict = dict()
  pav_dict = load_csv_file(filename = args.input)
  pav_dict = add_ka_ks(filename = args.ka_ks_file, pav_dict = pav_dict, arb_value = args.arb)
  ...
  pav_dict = parse_gff(filename = "/mnt/research/edgerpat_lab/AlanY/00_collab/02_core_pred/01_reference/Darmor_v81_assembly.augustus_masked.propermodels.sorted.renamed.gff", pav_dict = pav_dict)
  wkdir="/mnt/research/edgerpat_lab/AlanY/00_collab/02_core_pred/"
  pav_dict = add_go_terms(filename = wkdir + "/02_ka_ks/03_bna/01_data/bna_go_athal.tsv", pav_dict = pav_dict)

  pav_dict = add_aa_table(pav_dict = pav_dict)
