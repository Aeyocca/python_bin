...
#Alan E. Yocca
#09-16-20
#osat_auc_roc_curves_unbalanced.py
...
if __name__ == "__main__":
	...
	disp_subset = ml_df[ml_df.Membership == 0]
	ndisp = len(disp_subset)
	core_subset = ml_df[ml_df.Membership == 1].sample(n=ndisp, random_state=1)
	ml_df_balanced = disp_subset.append(core_subset)
	
	comb_list=[slice(0,46),slice(0,46),slice(0,46),slice(9,24),slice(9,24),slice(9,24)
				,slice(25,46),slice(25,46),slice(25,46),slice(0,8),slice(0,8),slice(0,8)]
	tag_list=["all","all","all","dinuc","dinuc","dinuc","aa","aa","aa","gene_feat","gene_feat","gene_feat",]
	...
	auc_roc_curves(ml_df = ml_df, features = ml_df[feature_list[comb_list[loop]]], 
					tag = "osat_bal" + tag_list[loop], model = model_list[loop])
