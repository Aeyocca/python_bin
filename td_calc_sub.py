...
def loop_hdf5():
  ...
  for i in range(0,len(chr_regions)):
    ...
    for chr_pos in positions[chr_regions[i][0]:chr_regions[i][1]]:
      ...
      #filter, SNPs binary encoded
      if (
        sum(snp_subset) == len(indicies_of_interest)
        or sum(snp_subset) == 0
      ):
        continue
        
      #if (
      #  sum(snp_subset) <= (len(indicies_of_interest) * 0.05)
      #  or sum(snp_subset) >= (len(indicies_of_interest) * 0.95)
      #):
      #  continue
      ...
      loop_snps = dendropy.StandardCharacterMatrix.get(
        data=phylip_string,
        schema="phylip")
      loop_fst = dendropy.calculate.popgenstat.tajimas_d(loop_snps)
      #future me problem to figure out how to convert to genomic location...
  ...
  with open("td_sub_srr.csv", "w", newline="") as fh:
    writer = csv.writer(fh)
    writer.writerows(all_fst)

#  with open("td_all_acc.csv", "w", newline="") as fh:
#    writer = csv.writer(fh)
#    writer.writerows(all_fst)
