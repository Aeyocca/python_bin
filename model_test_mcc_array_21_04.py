...
def calc_mcc(train_ml_df = "", test_ml_df = "", model = "", feature_list = ""):
	#extract features and scale
...
def calc_mcc_split(train_ml_df = "", test_ml_df = "", model = "", feature_list = ""):
	#extract features and scale
	train_target=np.ravel(train_ml_df['Membership'])
	train_features = train_ml_df[feature_list]
	train_features_scale = preprocessing.scale(train_features)
...
def loop_model_spec(df_dict = "", loop = 0):
	...
	for combo in combo_array:
		...
		print("Starting MCC " + model_tag + " " + train_species + " " + test_species)
		...
		if train_species.split("_")[0] == test_species.split("_")[0]:
			if train_species == test_species:
				...
				acc_stdev = calc_mcc_split(train_ml_df = df_dict[train_species],
								 		   test_ml_df = df_dict[test_species],
								 		   model = model,
								 		   feature_list = feature_list)				
		else:
			#cross data, can't do cv, just list acc, stdev = 0
			acc_stdev = calc_mcc(train_ml_df = df_dict[train_species],
								 test_ml_df = df_dict[test_species],
								 model = model,
								 feature_list = feature_list)
...
if __name__ == "__main__":
	...
	wkdir = "/mnt/research/edgerpat_lab/AlanY/00_collab/02_core_pred/07_ms_edits/06_MCC/"
	...
	output_table(out_dict = out_dict, 
		filename = wkdir + 'osat_bdis_bdisA_cv_mcc_table_21_04_' + args.slurm_array_id + '.txt',
		drop_header = True)
