#!/bin/python
#03-02-2021
#osat_bdis_kfold_model_test_prc_21_03.py
#Alan E. Yocca
...
parser.add_argument("--sub_feat", required=False, default = 0, help = "if specified, drop ortho ka/ks and dup type")
...
def calc_cv(ml_df = "", cv = 10, model = "", feature_list = []):
	...
	for train, test in cv:
		...
		pr = average_precision_score(target[test], probas_[:, 1])
		roc_auc.append(pr)
	
	auc_stdev = [statistics.mean(roc_auc), statistics.stdev(roc_auc)]
	return auc_stdev

def calc_prc(train_ml_df = "", test_ml_df = "", model = "", feature_list = ""):
	#extract features and scale
	...
	# Compute ROC curve and area the curve
	pr = average_precision_score(test_target, probas_[:, 1])
	roc_auc = pr
...
def calc_prc_split(train_ml_df = "", test_ml_df = "", model = "", feature_list = ""):
	#extract features and scale
	train_target=np.ravel(train_ml_df['Membership'])
	train_features = train_ml_df[feature_list]
	train_features_scale = preprocessing.scale(train_features)
	...
	# Compute ROC curve and area the curve
	pr = average_precision_score(target_test_test, probas_[:, 1])
	roc_auc = pr
			...
			for test_species in species_list:
				...
				if train_species.split("_")[0] == test_species.split("_")[0]:
					if train_species == test_species:
						...
						acc_stdev = calc_prc_split(train_ml_df = df_dict[train_species],
										 		   test_ml_df = df_dict[test_species],
										 		   model = model,
										 		   feature_list = feature_list)				
				else:
					#cross data, can't do cv, just list acc, stdev = 0
					acc_stdev = calc_prc(train_ml_df = df_dict[train_species],
										 test_ml_df = df_dict[test_species],
										 model = model,
										 feature_list = feature_list)
...
def output_table(out_dict = dict(), filename = ""):
	header = ["Model","Train","Test","AUC-PRC"]
...
if __name__ == "__main__":
	...
	#output
	filename = "osat_bdis_cv_prc_table.txt"
	if int(args.sub_feat):
		filename = "osat_bdis_cv_prc_sub_feat_table.txt"
	output_table(out_dict = out_dict, filename = "osat_bdis_cv_prc_table.txt")
