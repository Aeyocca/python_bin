...
#Alan E. Yocca
#02-28-23
#vaccpan_auc_roc_curves_array.py
...
parser.add_argument("--infile", required=True, help = "gene_stat file")
...
def cv_auc_roc_curve(model = "", features_scale = "", target = "", tag = "", cv = "cv"):
	...
	#want to output the data and replot myself laster, these just line plots
	#whats this look like? x/y coords but rounds also
	with open("auc_roc_data_" + tag + ".txt", "w") as out:
	  tmp = out.write("Fold\tFPR\tTPR\n")
	  for i in  range(len(fpr_list)):
	    tmp = out.write(str(i + 1) + "\t" + fpr_list[i] + "\t", tpr_list[i] + "\n")
...
if __name__ == "__main__":
	...
	#load data
	ml_df = pd.read_csv(args.infile, delimiter = "\t")
	#drop na
	ml_df = ml_df.dropna()
	#binarize
	ml_df['Membership'] = ml_df['Membership'].map({'Core': 1, 'Variable': 0})
	...
	feature_list = ["Length", "Intron_Length", "CDS_Length", "CDS_Count", "A", "C", "D",
	...
	auc_roc_curves(ml_df = ml_df, features = ml_df[feature_list[comb_list[loop]]], 
					tag = args.infile + "_" + tag_list[loop], model = model_list[loop])
	...
	auc_roc_curves(ml_df = ml_df_balanced, features = ml_df_balanced[feature_list[comb_list[loop]]], 
					tag = args.infile + "_bal_" + tag_list[loop], model = model_list[loop])
